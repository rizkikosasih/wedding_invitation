...
class event extends CI_Controller {
  ...
  public function list_gallery($id) {
    ...
    foreach ($result as $row) {
      ...
      $img_gallery = base_url("$this->dir_img/no-image.jpg");
      $img_gallery_hd = base_url("$this->dir_img/no-image.jpg");
      if ($row->images) {
        $img_gallery = base_url("$this->dir_img/gallery/x300_$row->images");
        $img_gallery_hd = base_url("$this->dir_img/gallery/x1920_$row->images");
      }

      $rows[] = [
        '<div class="text-center">'.++$nomor.'</div>',
        '<div class="text-center">
          <a class="lightbox title" href="'.$img_gallery_hd.'" data-title="Image Gallery">
            <img src="'.$img_gallery.'" class="img-rounded" alt="" width="100" height="auto">
          </a>
        </div>',
  ...
  public function list_wishes($id) {
    $event_id = decode64($id);
